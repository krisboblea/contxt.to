...
export default async function DashboardPage({
  ...
  searchParams: Promise<{ slug?: string; q?: string; create?: string }>
}) {
  const { slug, q, create } = await searchParams
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* List panel */}
      <div className={`${slug && !isCreating ? 'hidden md:flex' : 'flex'} w-full md:w-[380px] shrink-0 flex-col border-r border-[#F0EDE4] bg-white`}>
        <ContextList contexts={contexts} selectedSlug={slug ?? null} searchQuery={q ?? ""} />
      </div>
      {/* Detail / Create panel */}
      <div className={`${!slug && !isCreating ? 'hidden md:flex' : 'flex'} flex-1 flex-col overflow-hidden bg-[#FCF9F2]`}>
        <ContextDetail
          contexts={contexts}
